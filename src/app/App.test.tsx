// Routing contract for src/app/App.tsx — which hash resolves to which screen, and
...
//   file-level guard, like src/styles/tokens.test.ts — the values under test are CSS
...
import { moduleHref, SETUP_MODULE_ID, SETUP_ROUTE } from '../screens/HomeMap';
...
it('renders every screen inside one shell — exactly one <main> in the source', () => {
  const source = read('src/app/App.tsx').replace(/\/\*[\s\S]*?\*\//g, '');
...
it('leaves the bottom edge to the shell — no screen pads for it', () => {
  ...
  const screens = [
    ['src/screens/home.css', '.home-screen'],
    ['src/components/shelf.css', '.shelf-screen'],
    ['src/screens/settings.css', '.set-screen'],
    ['src/screens/setup.css', '.setup-screen'],
    ['src/screens/module.css', '.mod-screen'],
    ['src/screens/exercise.css', '.ex-screen'],
    ['src/components/loading.css', '.load-screen'],
  ] as const;
...
function shellBody() {
  const source = noComments(read('src/app/App.tsx'));
...
it('keeps the reset instant — no smooth scrolling for reduced motion to argue with', () => {
  expect(noComments(read('src/app/App.tsx'))).not.toMatch(/behavior:\s*'smooth'/);
  for (const path of [
    'src/app/app.css',
...
it('leaves scrolling to the shell — no screen resets or restores its own offset', () => {
  const screens = [
    'src/screens/HomeMap.tsx',
    'src/components/PhotocardShelf.tsx',
    'src/screens/Settings.tsx',
    'src/screens/SetupGuide.tsx',
    'src/screens/ModuleView.tsx',
    'src/screens/ExerciseView.tsx',
