...
// src/shell/bottomnav.css as text, the way src/styles/tokens.test.ts and the shell guard in
// src/app/App.test.tsx do: this suite has no jsdom, and a jsdom would resolve
...
import { HOME_ROUTE, SETTINGS_ROUTE, SHELF_ROUTE } from '../screens/HomeMap';

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../..');
const css = readFileSync(join(repoRoot, 'src/shell/bottomnav.css'), 'utf8').replace(
  /\/\*[\s\S]*?\*\//g,
  '',
);
...
it('offers exactly three destinations, in the order Map · Shelf · Settings', () => {
  ...
  expect(readFileSync(join(repoRoot, 'src/shell/BottomNav.tsx'), 'utf8')).not.toMatch(/'#\//);
});
...
it('gives every item a title alongside its aria-label, so a pointer user gets a tooltip too (#107)', () => {
  ...
  // Set in JSX, not behind a viewport check — src/shell/BottomNav.tsx has no
  // matchMedia and title is never inside a conditional.
  expect(readFileSync(join(repoRoot, 'src/shell/BottomNav.tsx'), 'utf8')).not.toMatch(/matchMedia/);
});
