...
// kinds of fact this repo's jsdom-free suite (src/app/App.test.tsx) can check
// without a document:
//
...
// 3. That the component is actually rendered from src/app/main.tsx, read as
...
describe('computeViewportContent', () => {
  it('locks zoom from a home-screen launch — while keeping viewport-fit=cover', () => {
    ...
    // nav's home-indicator padding (src/shell/bottomnav.css) in exactly the mode
...
it('is rendered once, near the root, in src/app/main.tsx', () => {
  const main = readFileSync(join(repoRoot, 'src/app/main.tsx'), 'utf8');
