...
```bash
npx -y @playwright/mcp@latest install-browser chrome-for-testing   # ~300 MB
```

That leaves the browser in Playwright's own cache, which differs by userland:
`~/.cache/ms-playwright` on Linux/the Pi, `~/Library/Caches/ms-playwright` on
macOS.

The repo intentionally has **no Playwright dependency** (02-engineering.md §2 keeps
the stack small and CI never needs a browser). Both probes find a Playwright on
the host through `scripts/playwright-host.mjs` — the repo's own copy if one
exists, otherwise an `npx`-cached copy whose pinned Chromium revision is
downloaded, looked for under **both** cache roots above so no environment
variable is needed on either host (#137). Override the search with
`PLAYWRIGHT_DIR=/path/to/node_modules/playwright`, or point
`PLAYWRIGHT_BROWSERS_PATH` at a browser cache to search that and nothing else.
When nothing is found the probe exits 2 and names every root it tried.
...
Dated passes live in `docs/`, numbered with the rest:

- [`docs/04-live-qa-2026-08-13.md`](04-live-qa-2026-08-13.md) — first full live pass
  (#28): every screen at 390px and 1280px, effort gate, persistence, offline,
  PWA metadata; 8 findings filed as #40–#47.
- [`docs/05-simplify.md`](05-simplify.md) — the said-once pass (2026-09-06): every screen
  measured at 360px before and after, in screens tall and where the one button sits.

## Screenshot index

`docs/screenshots/360-*.png`, the current screens at 360×740 (2× DPR), taken against
`npm run dev` after the said-once pass. `scripts/measure-screens.mjs` is the audit that
produced the numbers in `docs/05-simplify.md`; retake these whenever a screen changes.
...
02-engineering.md §4) resolves, so `--dump-dom` prints an empty `<div id="root">`;
